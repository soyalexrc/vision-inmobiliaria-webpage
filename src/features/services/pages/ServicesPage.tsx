...
function BannerComponent({item}: any) {
  ...
  return (
    ...
    >
      ...
      >
        <Typography variant='h1' align='center' color='#fff' sx={{letterSpacing: '3px'}}>Conoce nuestro
...
export default function ServicesPage() {
  ...
  React.useEffect(() => {
    ...
    if (location.hash) {
      ...
      const element: any = document.getElementById(location.hash.replace('#', ''));
      const yOffset = -80;
            ...
            <Grid item xs={12} md={9}>
              {
                [
                  ...
                  <Paper elevation={2} sx={{display: 'flex', my: 5, position: 'relative', flexDirection: largeScreen ? 'row' : 'column'}} id={element.id}>
                    <Box sx={{ position: 'absolute', ...(largeScreen ? {bottom: 0} : {top: 0}), left: 0, backgroundColor: 'primary.main', px: 2 }}>
                      <Typography variant='h3' color='#fff'>{index + 1}</Typography>
                    </Box>
                    <Box component='img' src={sampleImg} width={largeScreen ? 300 : '100%'}/>
                    <Box p={2}>
